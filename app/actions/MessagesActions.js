    ...
    if (window.shriek.stopscroll === false) {
      ...
      heightCalc
        .then(function (allheight) {
            if ((scroll + parseInt($('.msg__list > div:last-child').height())) >= (allheight - height) || force === true) {
              $('.msg__list').animate({ scrollTop: allheight }, 100, 'swing', function() {
                window.shriek.stopscroll = false;
              });
            } else {
              setTimeout(function() { window.shriek.stopscroll = false; }, 100);
            }
        })
